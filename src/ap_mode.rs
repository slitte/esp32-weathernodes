...
pub fn run(
    ...
    let srv_cfg = HttpConfig { stack_size: 12_288, ..Default::default() };
    ...
    // ── POST /save ────────────────────────────────────────────────────────────
    {
        ...
        server.fn_handler("/save", Method::Post, move |mut req| {
            ...
            if let Ok(mut g) = pending.lock() { *g = Some(cfg); }
    ...
    // ── POST /test_mqtt ───────────────────────────────────────────────────────
    {
        ...
        server.fn_handler("/test_mqtt", Method::Post, move |mut req| {
            ...
            let port   = params.get("mqtt_port")
                .and_then(|s| s.parse().ok()).unwrap_or(1883u16);
    ...
    server.fn_handler("/update", Method::Post, |mut req| {
        ...
        let json = match cfg.validate().and_then(|_| config::save_to_nvs(&cfg)) {
            ...
            Err(e) => format!(r#"{{"ok":false,"msg":"{}"}}"#, e.to_string().replace('"', "'")),
        };
        req.into_ok_response()?.write_all(json.as_bytes())?;
        Ok::<(), anyhow::Error>(())
    })?;
    ...
    // ── POST /sensor ──────────────────────────────────────────────────────────
    {
        ...
        server.fn_handler("/sensor", Method::Post, move |mut req| {
            let body   = read_body(&mut req);
            let params = parse_kv(&body);
            let sda = params.get("sda_pin").and_then(|s| s.parse().ok()).unwrap_or(21u8);
            let scl = params.get("scl_pin").and_then(|s| s.parse().ok()).unwrap_or(22u8);
...
fn test_wifi_connection(
    ...
    if ssid.is_empty() {
        return TestResult { ok: false, msg: "Kein SSID angegeben".into() };
    }
    let ssid_h: heapless::String<32> = match heapless::String::try_from(ssid) {
        Ok(s)  => s,
        Err(_) => return TestResult { ok: false, msg: "SSID zu lang (max 32 Zeichen)".into() },
    };
    let pass_h: heapless::String<64> = match heapless::String::try_from(password) {
        Ok(s)  => s,
        Err(_) => return TestResult { ok: false, msg: "Passwort zu lang (max 64 Zeichen)".into() },
    };
    ...
    let mut guard = match wifi_state.try_lock() {
        Ok(g)  => g,
        Err(_) => return TestResult { ok: false, msg: "Ein Test läuft bereits, bitte warten".into() },
    };
    ...
    let sta_cfg = ClientConfiguration { ssid: ssid_h, password: pass_h, ..Default::default() };
    let ap_cfg  = make_ap_cfg(&guard.ap_ssid);
    if let Err(e) = guard.wifi.set_configuration(&Configuration::Mixed(sta_cfg, ap_cfg)) {
        return TestResult { ok: false, msg: format!("Konfigurationsfehler: {e}") };
    }

    // Trigger non-blocking connect, then poll manually.
    if let Err(e) = guard.wifi.wifi_mut().connect() {
        return TestResult { ok: false, msg: format!("Verbindungsversuch fehlgeschlagen: {e}") };
    }
        ...
        match guard.wifi.is_connected() {
            Ok(true) => {
                ...
                loop {
                    ...
                    let ip = guard.wifi.wifi().sta_netif().get_ip_info()
                        .ok()
                        .map(|i| i.ip.to_string())
                        .unwrap_or_else(|| "0.0.0.0".into());
                    if ip != "0.0.0.0" {
                        return TestResult { ok: true, msg: format!("Verbunden! IP: {ip}") };
                    }
            ...
            Ok(false) => {}
            Err(e) => return TestResult { ok: false, msg: format!("Statusabfrage: {e}") },
        }
...
fn test_mqtt_connection(
    ...
    // try_lock: avoids blocking if a WiFi test is still running.
    {
        let guard = match wifi_state.try_lock() {
            Ok(g)  => g,
            Err(_) => return TestResult { ok: false, msg: "WiFi-Test läuft noch, bitte warten".into() },
        };
        match guard.wifi.is_connected() {
            Ok(true)  => {}
            Ok(false) => return TestResult {
                ok: false, msg: "WLAN nicht verbunden – zuerst WLAN testen".into(),
            },
            Err(e) => return TestResult { ok: false, msg: format!("WLAN-Status: {e}") },
        }
    }

    if server_addr.is_empty() {
        return TestResult { ok: false, msg: "Kein MQTT-Server angegeben".into() };
    }
    ...
    let (client, mut conn) = match EspMqttClient::new(&broker, &conf) {
        Ok(c)  => c,
        Err(e) => return TestResult { ok: false, msg: format!("Client-Erstellung: {e}") },
    };
        ...
        .spawn(move || loop {
            match conn.next() {
                Ok(ev) => match ev.payload() {
                    EventPayload::Connected(_) => { tx.try_send(true).ok();  break; }
                    EventPayload::Disconnected => { tx.try_send(false).ok(); break; }
                    EventPayload::Error(_)     => { tx.try_send(false).ok(); break; }
                    _                          => {}
                },
                Err(_) => { tx.try_send(false).ok(); break; }
            }
        });
    ...
    let conn_thread = match conn_thread {
        ...
        Err(e) => {
            drop(client);
            return TestResult { ok: false, msg: format!("Thread-Fehler: {e}") };
        }
    };

    let result = match rx.recv_timeout(Duration::from_secs(10)) {
        Ok(true)  => TestResult { ok: true,  msg: "MQTT-Verbindung erfolgreich!".into() },
        Ok(false) => TestResult { ok: false, msg: "Broker hat Verbindung abgelehnt".into() },
        Err(_)    => TestResult { ok: false, msg: "Timeout (10 s) – Broker nicht erreichbar".into() },
    };
...
fn read_sensor_json(i2c_mtx: &Arc<Mutex<I2C0>>, sda_pin: u8, scl_pin: u8) -> String {
    ...
    let json = match sensor::read_bme280(&bme_cfg, i2c0, sda, scl) {
        Ok((temp, pres, humi)) => {
            let t = temp.map(|v| format!("{v:.2}")).unwrap_or_else(|| "n/a".into());
            let p = pres.map(|v| format!("{v:.2}")).unwrap_or_else(|| "n/a".into());
            let h = humi.map(|v| format!("{v:.2}")).unwrap_or_else(|| "n/a".into());
...
fn parse_form(body: &str) -> Config {
    ...
    Config {
        device_name:   { let v = get("device_name"); if v.is_empty() { "weathernode".into() } else { v } },
        room:          get("room"),
        ...
        mqtt_topic:    { let v = get("mqtt_topic"); if v.is_empty() { "/data/nodes".into() } else { v } },
...
<script>
async function ct(url,fields,sid,bid){
...
function testWifi(){ct('/test_wifi',['wifi_ssid','wifi_password'],'st-wifi','btn-wifi');}
function testMqtt(){ct('/test_mqtt',['mqtt_server','mqtt_port','mqtt_user','mqtt_password'],'st-mqtt','btn-mqtt');}
...
<div class="card">
  ...
  <div class="br">
    ...
    <button type="button" id="sv-wifi" class="save" onclick="save('sv-wifi','st-wifi-s')">Speichern</button>
  </div>
  <span id="st-wifi" class="st"></span>
  <span id="st-wifi-s" class="st"></span>
</div>
...
<div class="card">
  ...
  <div class="br">
    <button type="button" class="dbtn" onclick="setDef([['mqtt_server',''],['mqtt_port','1883'],['mqtt_user',''],['mqtt_password',''],['mqtt_topic','/data/nodes'],['sleep_minutes','5']])">Standard</button>
