    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        if self.sda_pin > 33 {
            return Err(ConfigError::Validation(
                format!("SDA GPIO{} is input-only on ESP32 (valid: 0–33)", self.sda_pin)));
        }
        if self.scl_pin > 33 {
            return Err(ConfigError::Validation(
                format!("SCL GPIO{} is input-only on ESP32 (valid: 0–33)", self.scl_pin)));
        }
        if self.sda_pin == self.scl_pin {
            return Err(ConfigError::Validation(
                format!("SDA and SCL must be different pins (both GPIO{})", self.sda_pin)));
        }
        if self.send_battery {
            if !(32..=39).contains(&self.adc_pin) {
                return Err(ConfigError::Validation(
                    format!("ADC GPIO{} is not an ADC1 channel (valid: 32–39)", self.adc_pin)));
            }
            if self.adc_pin == self.sda_pin || self.adc_pin == self.scl_pin {
                return Err(ConfigError::Validation(
                    format!("ADC GPIO{} conflicts with I²C pins (SDA={} SCL={})",
...
fn nvs_bool(nvs: &EspNvs<NvsDefault>, key: &str) -> Result<bool, ConfigError> {
    Ok(get_str(nvs, key)?.map_or(true, |s| s != "0"))
}
...
pub fn load_from_nvs() -> Result<Option<Config>, ConfigError> {
    ...
    Ok(Some(Config {
        ...
        mqtt_port:     get_str(&nvs, K_MQTT_PORT)?.and_then(|s| s.parse().ok()).unwrap_or(1883),
        ...
        sleep_minutes: get_str(&nvs, K_SLEEP_MINS)?.and_then(|s| s.parse().ok()).unwrap_or(5),
        bme280_addr:   0x76,
        sda_pin:       get_str(&nvs, K_SDA_PIN)?.and_then(|s| s.parse().ok()).unwrap_or(21),
        scl_pin:       get_str(&nvs, K_SCL_PIN)?.and_then(|s| s.parse().ok()).unwrap_or(22),
        adc_pin:       get_str(&nvs, K_ADC_PIN)?.and_then(|s| s.parse().ok()).unwrap_or(33),
...
pub fn save_to_nvs(c: &Config) -> Result<(), ConfigError> {
    ...
    set_str(&mut nvs, K_SEND_TEMP, if c.send_temperature { "1" } else { "0" })?;
    set_str(&mut nvs, K_SEND_PRES, if c.send_pressure    { "1" } else { "0" })?;
    set_str(&mut nvs, K_SEND_HUMI, if c.send_humidity    { "1" } else { "0" })?;
